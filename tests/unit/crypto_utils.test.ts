...
describe('crypto_utils', () => {
    ...
    test('sign - recover correct address from signature', () => {
        const PRIVATE_KEY = '0x526c9f9fe2fc41fb30fd0dbba1d4d76d774030166ef9f819b361046f5a5b4a34'
        const wallet = new Wallet(PRIVATE_KEY)
        const MESSAGE_HASH = solidityPackedKeccak256(["string"], ["test message"])
        ...
        const recoveredAddress = recoverAddress(MESSAGE_HASH, { r, s, v })
    ...
    test('signInputText - recover correct address from signature', () => {
        const wallet = new Wallet(TEST_CONSTANTS.PRIVATE_KEY)
        const contractAddress = TEST_CONSTANTS.CONTRACT_ADDRESS
        const functionSelector = TEST_CONSTANTS.FUNCTION_SELECTOR
        const ct = BigInt(12345)
        ...
        const messageHash = solidityPackedKeccak256(
            ...
            [wallet.address, contractAddress, functionSelector, ct]
        )
        ...
        const recoveredAddress = recoverAddress(messageHash, { r, s, v })
        expect(recoveredAddress.toLowerCase()).toBe(wallet.address.toLowerCase())
    })
    ...
    describe('decryptUint', () => {
        test('decrypt the ciphertext of an arbitrary unsigned integer', () => {
            ...
            const { ciphertext } = prepareIT(PLAINTEXT, sender, TEST_CONSTANTS.CONTRACT_ADDRESS, TEST_CONSTANTS.FUNCTION_SELECTOR)
    ...
    describe('buildStringInputText', () => {
        ...
        testBuildStringInputText('🚀 Emojis', 'emojis (multi-byte)')
        testBuildStringInputText('שלום עולם', 'Hebrew (multi-byte + RTL)')
        testBuildStringInputText('العالم العربي', 'Arabic (multi-byte + RTL)')
        testBuildStringInputText('你好世界', 'Chinese (multi-byte)')
    ...
    describe('decryptString', () => {
        test('decrypt the ciphertext of an arbitrary string', () => {
            const PLAINTEXT = 'Hello, world!'
            const { ciphertext } = buildStringInputText(PLAINTEXT, createTestSender(), TEST_CONSTANTS.CONTRACT_ADDRESS, TEST_CONSTANTS.FUNCTION_SELECTOR)
        ...
        // UTF-8 Round-trip
        testDecryptStringRoundTrip('🚀 Emojis and ✨ Sparkles', 'round-trip: emojis')
        testDecryptStringRoundTrip('עברית', 'round-trip: Hebrew')
        testDecryptStringRoundTrip('日本語', 'round-trip: Japanese')
        testDecryptStringRoundTrip('𐀀 Linear B', 'round-trip: 4-byte UTF-8')

        // Boundary Round-trip
        testDecryptStringRoundTrip('1234567', 'round-trip: 7 bytes')
        testDecryptStringRoundTrip('12345678', 'round-trip: 8 bytes')
        testDecryptStringRoundTrip('123456789', 'round-trip: 9 bytes')
        testDecryptStringRoundTrip('123456789012345', 'round-trip: 15 bytes')
        testDecryptStringRoundTrip('1234567890123456', 'round-trip: 16 bytes')
        testDecryptStringRoundTrip('12345678901234567', 'round-trip: 17 bytes')
    })
    ...
    describe('generateRSAKeyPair', () => {
        ...
        test('generateRSAKeyPair - validates key properties', () => {
            const { publicKey, privateKey } = generateRSAKeyPair()

            // Validate Private Key
            const forgePrivKey = forge.pki.privateKeyFromAsn1(forge.asn1.fromDer(forge.util.createBuffer(privateKey as any)))
            expect(forgePrivKey.n.bitLength()).toBe(2048)

            // Validate Public Key
            const forgePubKey = forge.pki.publicKeyFromAsn1(forge.asn1.fromDer(forge.util.createBuffer(publicKey as any)))
            expect(forgePubKey.n.bitLength()).toBe(2048)

            // Verify they are a pair
            const message = 'test rsa message'
            const encrypted = forgePubKey.encrypt(message)
            const decrypted = forgePrivKey.decrypt(encrypted)
